...
Where a provider lacks an operator, the compiler rewrites the filter instead of failing. See [Provider differences](/docs/providers/differences) for the two cases where the semantics diverge.
