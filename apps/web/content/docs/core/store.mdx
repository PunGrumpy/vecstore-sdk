...
Create a store with the adapter for your provider: `createQdrantStore`, `createPgvectorStore`, or `createPineconeStore`. Each takes the native client under `client`.
...
| `provider` | `"qdrant"`, `"pgvector"`, or `"pinecone"`. |
| `raw` | The client you passed in, with its original type. |

## Index

Every provider gets namespaces. Pinecone has them natively. On Qdrant and pgvector the adapter emulates them, as described on each provider page.
...
Metadata values are `string`, `number`, `boolean`, or `string[]`. That is the intersection of what the three providers accept, so a record that upserts on one provider upserts on all of them.

## Scores

Each provider returns its native score for the index metric. Higher is better for `cosine` and `dot`. Lower is better for `euclidean`.
