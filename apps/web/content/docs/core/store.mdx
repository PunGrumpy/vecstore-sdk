...
Create a store with the adapter for your provider: `createQdrantStore`, `createPgvectorStore`, `createPineconeStore`, or `createUpstashStore`. Each takes the native client under `client`.
...
| `provider` | `"qdrant"`, `"pgvector"`, `"pinecone"`, or `"upstash"`. |
| `raw` | The client you passed in, with its original type. |

## Index

Every provider gets namespaces. Pinecone has them natively. On Qdrant, pgvector, and Upstash the adapter emulates them, as described on each provider page.
...
Metadata values are `string`, `number`, `boolean`, or `string[]`. That is the intersection of what the four providers accept, so a record that upserts on one provider upserts on all of them.
