...
For pgvector, install `pg` instead. For Pinecone, install `@pinecone-database/pinecone`. For Upstash Vector, install `@upstash/vector`. For Supabase, install `@supabase/supabase-js` and run the [SQL install file](/docs/providers/supabase#install-the-sql-functions) once.
