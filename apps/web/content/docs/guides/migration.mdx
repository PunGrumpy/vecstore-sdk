...
| Concern | Qdrant | pgvector and Supabase | Pinecone | Upstash | Vectorize |
| --- | --- | --- | --- | --- | --- |
| Index | Collection | Table | Index | Namespace | Index |
| Namespace | `_namespace` payload key | `namespace` column | Native | `_namespace` metadata key, or a real namespace under `namespaceMode: "native"` | Native |
| Id | UUID, hashed from your id when needed. The original id is kept in `_id`. | `id text` | Native | Prefixed with the namespace. The original id is kept in `_id`. Unchanged under `namespaceMode: "native"`. | UUID, hashed from your id when needed. The original id is kept in `_id`. |
| Metadata | Payload | `metadata jsonb` | Metadata | Metadata | Metadata |
| Metric | Set on the collection | Read from the HNSW index operator class | Set on the index | Set on the Upstash index | Set on the index |
...
Qdrant, pgvector, Pinecone, and Vectorize return the provider's native score for the index metric, and Upstash normalizes every metric to the range 0 to 1. A threshold tuned on one provider does not carry over. See the score table in [Stores and indexes](/docs/core/store#scores) and check the threshold against real data.
