...
The pgvector and Supabase adapters also run against PGlite, a Postgres build for WebAssembly, inside the unit suite. `test/postgres` installs `sql/supabase.sql` into it, runs the conformance cases against both adapters, and checks that `vecstore_filter_sql` returns the same rows as `compilePgvectorFilter` for every filter in a corpus. No server or Docker is needed.
