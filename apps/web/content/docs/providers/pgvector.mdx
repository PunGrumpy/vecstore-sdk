...
The store caches the metric per table after the first query and forgets it when `createIndex` or `deleteIndex` on the same store touches that table. If another process recreates a table with a different metric, create a new store.
