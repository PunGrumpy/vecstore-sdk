...
| Record | Stored id |
| --- | --- |
| In the default namespace, id at most 64 bytes and not a version 8 UUID | Your id, and no `_id` key |
...
The adapter sends up to 1,000 records per upsert request as NDJSON and runs up to four batches at a time.
