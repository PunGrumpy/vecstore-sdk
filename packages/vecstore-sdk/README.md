...
An index is a collection. Qdrant accepts only UUID or integer point ids. The adapter hashes other ids to a deterministic UUID and stores your id in the `_id` payload key. Lowercase UUID ids in the default namespace pass through unchanged, unless the UUID is version 8, the version the adapter's own hashed ids use; those are hashed too, so a raw id can never collide with a hashed one. Namespaces live in the `_namespace` payload key, and `createIndex` adds a tenant keyword index on it. The adapter strips both keys from returned metadata. A namespace cannot contain `/`, and `upsert` rejects a record whose metadata sets a reserved key with `invalid_argument`.
